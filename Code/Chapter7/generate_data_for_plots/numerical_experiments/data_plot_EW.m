function [EW] = data_plot_EW(d,lambda,SCV,f,type,Delta,RepMax,threshold,Bsize,tol,good_threshold, jtype, q, EX1, k_PH)
...
if jtype == 1
    q=1; EX1=1; k_PH=1; % set arbitrarily
end
...
for i=1:length(iter)
    ...
    [k_indxs, l_indxs, r, greater, greater_or_eq] = sorted_kl_indxs(SCV, f,type,Delta,RepMax,Bsize,threshold, jtype, q, EX1, k_PH);
    if i == 1
        [lambdas] = init_lambda(r, Bsize);
    end
    [EW(i), lambdas, ~, ~, ~] = simple_kl_PH(d,lambda,SCV,f,Delta,Bsize,r,k_indxs, l_indxs, greater, greater_or_eq, tol, lambdas, jtype, q, EX1, k_PH); 
end
