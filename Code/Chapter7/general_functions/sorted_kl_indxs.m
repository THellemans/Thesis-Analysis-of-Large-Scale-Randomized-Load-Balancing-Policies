function [k_indxs, l_indxs, r, greater, greater_or_eq, xi] = sorted_kl_indxs(SCV, f,type,Delta,RepMax,Bsize,threshold, jtype, q, EX1, k_PH)
% type = 1 : SQ(d)-RTB
% type = 2 : SQ(d)-RE(T)
...
if jtype == 1
    EX=1;
    [alpha, A, ~, ~] = hyperexponential_init(EX, SCV, f);
elseif jtype == 2
...
elseif type == 7
    ...
    for i=1:length(k_indxs)
        if exist('data','var')
            xi(i)=E_cond_data(data, b(k_indxs(i))) - b(k_indxs(i)) + (l_indxs(i)-1);
        else
