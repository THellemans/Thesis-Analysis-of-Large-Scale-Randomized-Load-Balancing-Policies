function [EW, lambdas, probs, probs2, cumProbs, error] = simple_kl_PH(d,lambda,SCV,f,Delta,Bsize,r,k_indxs, l_indxs, greater, greater_or_eq, tol, lambdas, jtype, q, EX1, k_PH)
% lambda : arrival rate
...
if jtype == 1
    EX=1;
    [alpha, A, mu, ~] = hyperexponential_init(EX, SCV, f);
elseif jtype == 2
    [alpha, A, mu] = erlang_plus_hyperexponential_init(q, k_PH, SCV, f);
elseif jtype == 3
    [alpha, A, mu] = hyper_erlang_init(q, k_PH, EX1);
end
...
while true
    ...
    for k=0:r
        F(:,k*Bm+1:(k+1)*Bm)=kron(-diag(lambdas(k+1,:))+diag(lambdas(k+1,1:Bsize-1),1),eye(m))+kron(eye(Bsize),A);
        if (k < r)
    ...
    if run >= 20 && ( error(run) < tol || run >= 200 || error(run) > error(run-1) )
        break
    end
    old_probs=probs;
